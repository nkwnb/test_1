...
# 论文名称：FFT-based Dynamic Token Mixer for Vision
...
def resize_complex_weight(origin_weight, new_h, new_w):
    ...
    new_weight = (
        torch.nn.functional.interpolate(
            origin_weight, size=(new_h, new_w), mode="bicubic", align_corners=True
        )
        .permute(0, 2, 3, 1)
...
class DynamicFilter(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.reweight = Mlp(
            dim, reweight_expansion_ratio, num_filters * self.med_channels
        )
        self.complex_weights = nn.Parameter(
            torch.randn(
                self.size, self.filter_size, num_filters, 2, dtype=torch.float32
            )
            * 0.02
        )
...
if __name__ == "__main__":
    # 初始化模型，设置输入通道数为128，过滤器尺寸为20x20
    model = DynamicFilter(dim=128, size=20).cuda()

    # 创建一个随机初始化的输入张量，维度为 [batch_size, channels, height, width]
    # 这里的 batch_size=3, channels=128, height=20, width=20
    input_tensor = torch.rand(3, 128, 20, 20).cuda()

    # 通过模型传递输入张量，获取输出
    output = model(input_tensor)

    # 打印输入和输出张量的大小，以验证维度的一致性
