...
class DSConv_Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.snc1 = DSConv(c2, c2, 3, 1, morph=1, if_offset=True)
        self.snc2 = DSConv(c2, c2, 3, 1, morph=0, if_offset=True)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """'forward()' applies the YOLOv5 FPN to input data."""
        return (
            x + self.snc2(self.snc1(self.cv2(self.cv1(x))))
            if self.add
            else self.snc2(self.snc1(self.cv2(self.cv1(x))))
        )
...
class DSConv_C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            DSConv_Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0)
            for _ in range(n)
        )
