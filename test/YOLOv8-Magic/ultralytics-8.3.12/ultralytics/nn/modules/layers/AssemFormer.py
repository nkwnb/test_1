...
class BaseConv2d(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.Bn = (
            nn.BatchNorm2d(out_channels, eps=0.001, momentum=Momentum)
            if BNorm
            else nn.Identity()
        )
...
class BaseFormer(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        ViTSELayer = (
            ViTSELayer(InChannels, **kwargs)
            if ViTSELayer is not None
            else nn.Identity()
        )
    ...
    def buildAttnLayer(
        ...
        if isinstance(FfnMult, Sequence) and len(FfnMult) == 2:
            DimFfn = (
                np.linspace(FfnMult[0], FfnMult[1], NumAttnBlocks, dtype=float)
                * DimModel
            )
        ...
        GlobalRep = [
            LinearAttnFFN(
                DimModel, DimFfn[block_idx], AttnDropRate, DropRate, FfnDropRate
            )
