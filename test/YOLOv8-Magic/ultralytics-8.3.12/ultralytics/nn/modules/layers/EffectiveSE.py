...
__all__ = ("EffectiveSEModule", "C3_EffectiveSE", "C2f_EffectiveSE")
...
class EffSE_Bottleneck(nn.Module):
...
class C3_EffectiveSE(nn.Module):
    # CSP Bottleneck with 3 convolutions and 1 EffectiveSE. by CSDN迪菲赫尔曼
    def __init__(
        ...
        self.m = nn.Sequential(
            *(EffSE_Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n))
        )
...
class EffectiveSE_Bottleneck_(nn.Module):
    def __init__(
        ...
        self.effse = EffectiveSEModule(c2, add_maxpool=False, gate_layer="hard_sigmoid")
        self.add = shortcut and c1 == c2
...
class C2f_EffectiveSE(nn.Module):
    """CSP Bottleneck with 2 convolutions and 1 EffectiveSE. by csdn迪菲赫尔曼"""
