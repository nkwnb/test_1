...
__all__ = ("BasicStage", "PatchEmbed_FasterNet", "PatchMerging_FasterNet")
...
class BasicStage(nn.Module):
    def __init__(
        ...
        super().__init__()
        dpr = [x.item() for x in torch.linspace(0, 0.0, sum([1, 2, 8, 2]))]
        blocks_list = [
            MLPBlock(
                dim=dim,
                n_div=n_div,
...
class PatchEmbed_FasterNet(nn.Module):
    def __init__(
        self, in_chans, embed_dim, patch_size, patch_stride, norm_layer=nn.BatchNorm2d
    ):
        super().__init__()
        self.proj = nn.Conv2d(
...
class PatchMerging_FasterNet(nn.Module):
    def __init__(self, dim, out_dim, k, patch_stride2, norm_layer=nn.BatchNorm2d):
        ...
        self.reduction = nn.Conv2d(
            dim, out_dim, kernel_size=k, stride=patch_stride2, bias=False
        )
        if norm_layer is not None:
